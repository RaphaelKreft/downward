...
REVISION = "98239cfcff30150460b397ce9edf4ae02c0a6251"
REPO = os.environ["DOWNWARD_REPO"]
BENCHMARKS_DIR, SUITE, ENVIRONMENT = setup_environment()

# time in seconds
max_state_caps = [3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]
CONFIGS = []

for state_cap in max_state_caps:
    CONFIGS.append(common_setup.IssueConfig(f"daPrecomp-{state_cap}", ["--search", f"astar(domain_abstraction(precalculation=true, max_states={state_cap},split_selector=min_states_gain,singlevaluesplit=false))"]))
    ...
    CONFIGS.append(common_setup.IssueConfig(f"daOTF-{state_cap}", ["--search", f"astar(domain_abstraction(precalculation=false, max_states={state_cap},split_selector=min_states_gain,singlevaluesplit=false))"]))
    CONFIGS.append(common_setup.IssueConfig(f"daOTF-{state_cap}-sv-minStateGain", ["--search", f"astar(domain_abstraction(precalculation=false, max_states={state_cap},singlevaluesplit=true,split_selector=min_states_gain))"]))
